...
r = requests.get(BASE_URL + 'playlists/' + playlist_id, headers=auth_header, params={'market':'US'})
...
r = requests.get(BASE_URL + 'artists/' + uziKey + "/top-tracks/", headers=auth_header, params={'market':'US'})
data = r.json()
uziSongs = []
...
for artistKey in artists.keys():
    ...
    r = requests.get(BASE_URL + 'artists/' + artistKey + "/top-tracks/", headers=auth_header, params={'market':'US'})
    data = r.json()
